...
function App() {
 ...
 if(mode[0] === "Read") {
    ...
    _article = <_CreateContent onSubmit={function(newTitle, newDesc) {
    ...
    _article = <_UpdateContent item={content} onSubmit={
      function(_id, _title, _desc) {
        let contentTOC = Array.from(items_TOC);
        let contentReadContent = Array.from(items_readContent);
        
        contentTOC[content.id-1] = {id:content.id, title:_title, desc:_desc};
  ...
  return (
    <div className="App">
      ...
      <_TOC items={items_TOC} onChangePage={function(mode) { setMode(mode); }}/>
      <_Control onChangeMode={function(mode) { setMode(mode) }} />
      {_article}
    </div>
  );
}
